...
def mix_columns(state):
    for i in range(4):
        col = state[:, i]
        state[:, i] = [
            (2 * col[0]) ^ col[1] ^ col[2] ^ (3 * col[3]),
            (2 * col[1]) ^ col[2] ^ col[3] ^ (3 * col[0]),
            (2 * col[2]) ^ col[3] ^ col[0] ^ (3 * col[1]),
            (2 * col[3]) ^ col[0] ^ col[1] ^ (3 * col[2]),
        ]
    return state
...
def encrypt(plaintext, key):
    ...
    state = np.array(data).reshape(4, 4)  
